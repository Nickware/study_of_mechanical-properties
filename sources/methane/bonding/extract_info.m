% filepath: /tmp/test/methane/bonding/extract_info.m
...
while ~feof(fid)
    ...
    % Detectar el inicio de la tabla de minimización de energía
    ...
    % Detectar el inicio de la tabla de dinámica molecular
    if ~isempty(strfind(line, 'Step          Temp          E_pair         E_mol         TotEng'))
        fgetl(fid); % Saltar línea
        while true
            data_line = fgetl(fid);
            if isempty(data_line) || ~ischar(data_line) || ~isempty(strfind(data_line, 'Loop time'))
